...
describe('RouteMatcher', () => {
      ...
      const routes: RouteConfig<string, void>[] = [
        {
          ...
          data: 'parent',
          children: [
            { path: ['child-static', ':child-dynamic'], data: 'child' },
          ],
        },
      ];
      const matcher = new RouteMatcher(routes);
  ...
  describe('getRouteConfig', () => {
    it('should return route configs', () => {
      ...
      // when
      const parentRouteConfig = matcher.getRouteConfig(0);
      const childRouteConfig = matcher.getRouteConfig(1);

      // then
      expect(parentRouteConfig?.data).toBe('parent');
      expect(childRouteConfig?.data).toBe('child');
    });
  });
});
