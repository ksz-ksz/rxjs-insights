...
export function createQueryEffect(
  name: string,
  actions: Actions,
  scheduler: SchedulerLike,
  resourceActions: ResourceActionTypes,
  queries: QueriesDef<{ [key: string]: Fn }>
) {
          ...
          mergeMap((queryActions) =>
            queryActions.pipe(
              ...
              exhaustMap(({ payload: { queryKey, queryArgs, queryState } }) => {
                ...
                return query.queryFn(queryArgs).pipe(
                  last(),
                  ...
                  connect((result) =>
                    merge(
                      ...
                      query.dispatch?.(result, queryArgs) ?? EMPTY
                    )
                  ),
                  takeUntil(
                    queryActions.pipe(
                      filter(is(resourceActions.queryCancelled))
                    )
                  )
                );
              })
            ...
            switchMap(({ payload: { queryKey, queryArgs, queryState } }) => {
              if (
                ...
                const cacheTimestamp =
                  getQueryCacheTimestamp(queryState) ?? now;
...
function getQueryDef(
  queries: QueriesDef<{ [key: string]: Fn }> | undefined,
  queryKey: string
): QueryDef<Fn> {
